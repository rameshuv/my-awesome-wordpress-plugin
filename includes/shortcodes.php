...
add_shortcode('bhg_user_profile', function($atts){
    ...
    <div class="bhg-box">
        ...
        // Hunts participated
        global $wpdb;
        $rows = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT h.id, h.title, h.status FROM {$wpdb->prefix}bhg_bonus_hunts h INNER JOIN {$wpdb->prefix}bhg_guesses g ON g.hunt_id=h.id WHERE g.user_id=%d ORDER BY h.id DESC", $uid));
        if ($rows): ?>
...
add_shortcode('bhg_active_hunt', function($atts){
    global $wpdb;
    $hunts = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}bhg_bonus_hunts WHERE status=%s ORDER BY id DESC", 'open'));
    if (!$hunts) return '<div class="bhg-empty">'.esc_html__('No active bonus hunts right now.','bonus-hunt-guesser').'</div>';
    ...
    foreach ($hunts as $hunt):
        ...
        <div class="bhg-box">
            ...
            <p><em><?php esc_html_e('Starting Balance','bonus-hunt-guesser'); ?>:</em> <?php echo esc_html(function_exists('wc_price') ? wc_price($hunt->starting_balance) : number_format_i18n($hunt->starting_balance,2)); ?></p>
            <?php if ($desc): ?><p><?php echo esc_html($desc); ?></p><?php endif; ?>
            <?php if ($prizes): ?><p><strong><?php esc_html_e('Prizes','bonus-hunt-guesser'); ?>:</strong> <?php echo esc_html($prizes); ?></p><?php endif; ?>

            <?php if ($uid): 
                $user_guess = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}bhg_guesses WHERE user_id=%d AND hunt_id=%d", $uid, $hunt->id));
                if ($user_guess): ?>
                    <p><strong><?php esc_html_e('Your guess','bonus-hunt-guesser'); ?>:</strong> <?php echo esc_html(number_format_i18n($user_guess->guess_value, 2)); ?></p>
                <?php else: ?>
                ...
                $guesses = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}bhg_guesses WHERE user_id=%d AND hunt_id=%d ORDER BY id DESC", $uid, $hunt->id));
                if ($guesses): ?>
                    <h4><?php esc_html_e('Your guesses','bonus-hunt-guesser'); ?></h4>
                    <table class="bhg-leaderboard">
                        <thead><tr><th><?php esc_html_e('Guess','bonus-hunt-guesser'); ?></th><th><?php esc_html_e('Date','bonus-hunt-guesser'); ?></th></tr></thead>
                        <tbody>
                            <?php foreach ($guesses as $g): ?>
                                <tr><td><?php echo esc_html(number_format_i18n($g->guess_value,2)); ?></td><td><?php echo esc_html(date_i18n(get_option('date_format').' '.get_option('time_format'), strtotime($g->created_at))); ?></td></tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                <?php endif; ?>
...
add_shortcode('bhg_guess_form', function($atts){
    ...
    global $wpdb;
    $hunts = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}bhg_bonus_hunts WHERE status=%s ORDER BY id DESC", 'open'));
    if (!$hunts) return '<div class="bhg-empty">'.esc_html__('No active hunts to guess for.','bonus-hunt-guesser').'</div>';
...
add_shortcode('bhg_tournaments', function($atts){
    ...
    $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}bhg_tournaments WHERE status=%s ORDER BY id DESC", $status));

    ob_start(); ?>
    <div class="bhg-box">
        <form method="get" class="bhg-field">
            <?php foreach ($_GET as $k=>$v){ if ($k==='status') continue; ?>
...
add_shortcode('bhg_user_guesses', function($atts){
    if (!is_user_logged_in()) return '<div class="bhg-box">'.esc_html__('Please log in.','bonus-hunt-guesser').'</div>';
    global $wpdb;
    $uid = get_current_user_id();
    $rows = $wpdb->get_results($wpdb->prepare("SELECT g.*, h.title as hunt_title, h.status as hunt_status FROM {$wpdb->prefix}bhg_guesses g LEFT JOIN {$wpdb->prefix}bhg_bonus_hunts h ON h.id=g.hunt_id WHERE g.user_id=%d ORDER BY g.id DESC", $uid));
    ob_start(); ?>
    <div class="bhg-box">
        <h3><?php esc_html_e('Your Guesses','bonus-hunt-guesser'); ?></h3>
        <?php if ($rows): ?>
            <table class="bhg-leaderboard">
                <thead><tr><th><?php esc_html_e('Hunt','bonus-hunt-guesser'); ?></th><th><?php esc_html_e('Guess','bonus-hunt-guesser'); ?></th><th><?php esc_html_e('Status','bonus-hunt-guesser'); ?></th><th><?php esc_html_e('Date','bonus-hunt-guesser'); ?></th></tr></thead>
                <tbody>
                    <?php foreach ($rows as $r): ?>
                        <tr>
...
add_shortcode('bhg_winner_notifications', function($atts){
    ...
    $table_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=%s AND TABLE_NAME=%s", DB_NAME, $winners_table));
    if ($table_exists){
        $rows = $wpdb->get_results("SELECT h.id as hunt_id, h.title, w.user_id, w.position FROM {$wpdb->prefix}bhg_bonus_hunts h INNER JOIN {$wpdb->prefix}bhg_hunt_winners w ON w.hunt_id=h.id WHERE h.status='closed' ORDER BY h.id DESC");
    } else {
        $rows = $wpdb->get_results($wpdb->prepare("SELECT id as hunt_id, title FROM {$wpdb->prefix}bhg_bonus_hunts WHERE status=%s ORDER BY id DESC", 'closed'));
    }
    ...
    <div class="bhg-box">
        ...
        <?php if ($rows): ?>
            <ul class="bhg-list">
            <?php foreach ($rows as $r): $u = isset($r->user_id) && $r->user_id ? get_userdata($r->user_id) : null; ?>
                <li>
                    <strong><?php echo esc_html($r->title ?: ('#'.$r->hunt_id)); ?></strong>
                    <?php if ($u): ?>
                        — <?php echo esc_html($u->display_name ?: $u->user_login); ?><?php if (!empty($r->position)) echo ' ('.intval($r->position).')'; ?>
                    <?php else: ?>
